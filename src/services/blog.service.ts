...
export class BlogService {
  ...
  // Update a blog post
  public async updatePost(id: string, updateData: Partial<IBlog>, authorId: string): Promise<IBlog> {
    const blog = await this.blogRepo.findById(id);
    if (!blog) {
      throw new NotFoundError('Blog post not found');
    }

    // Verify authorship or admin role (can be bypassed by admin checks)
    if (blog.author._id.toString() !== authorId) {
      // Allow controllers to check if user is admin
    }
  ...
  // Get blogs with pagination (Public: only published, Admin: all)
