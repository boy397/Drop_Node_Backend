...
export class CategoryService {
    ...
    nodeMap.forEach((node) => {
      if (node.parentCategory) {
        ...
        if (parent) {
          ...
          // If parent is not in the map, treat as root (dangling child)
          roots.push(node);
        }
      } else {
        roots.push(node);
  ...
  public async deleteCategory(id: string): Promise<void> {
    ...
    // TODO: When ProductRepository is built, check if any products belong to this category!
    // For now, let's proceed with category deletion.
  ...
  // Helper to clear category tree cache
